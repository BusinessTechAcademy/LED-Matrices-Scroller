...
while True:
    ...
    else: # If random quotes and colors is not enabled then choose the color and quote that are on top.
    ...
    if animations_enabled == True: # If our testing / debug varible is true then we let the lines for our animation system run.
        ...
        if set_animation == 1: # If the top value equals to "1" we run our first animation.
            matrixportal.set_text(quotes[quote_index]) # Display the text on our scrolling text box (results in a plain scrolling text animation)
